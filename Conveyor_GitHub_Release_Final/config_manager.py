...
class ConfigManager:
    ...
    def get_modifier_keys(self):
        ...
        for mod_str in mod_list:
            ...
            # Split by common separators if it's a single string with multiple modifiers
            parts = mod_str.replace("+", " ").replace(",", " ").split()
            for part in parts:
                if "alt_l" in part: keys.append(keyboard.Key.alt_l)
                elif "alt_r" in part: keys.append(keyboard.Key.alt_r)
                elif "win" in part or "cmd" in part: keys.append(keyboard.Key.cmd_l)
                elif "ctrl" in part: keys.append(keyboard.Key.ctrl_l)
                elif "shift" in part: keys.append(keyboard.Key.shift)
